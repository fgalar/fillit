...
/*   Updated: 2019/05/22 18:20:36 by mdavid           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * DESCRIPTION :
 *     Vérifie quand c'est possible s'il y a des blocs voisins juste au dessus et
 *     juste en dessous.
 * RETOUR :
 ...
 * PARAMETRES : char *buffer (le possible tetramino), int i (la position du bloc).
 * DESCRIPTION :
 *     Vérifie quand c'est possible s'il y a des blocs voisins juste à gauche et
 *     juste à droite.
 * RETOUR :
 *     0/1/2 -> le nombre de blocs voisins.
 */
 ...
 *              buffer |string, contenant le 21 char, donc une possible piece de tetramino|. 
 * DESCRIPTION :
...
int     ft_table_neighbours(int **table, char *buffer)
{
    ...
    while (buffer[i] && i < 19)
    {
        while (buffer[i] != '#' && i < 19)
            i++;
        if (buffer[i] == '#' && i < 19)
            table[i / 5][i % 5] = ft_blocs_updown(buffer, i) + ft_blocs_leftright(buffer, i);
        i++;
    }
    i = 0;
    while (i < 19)
